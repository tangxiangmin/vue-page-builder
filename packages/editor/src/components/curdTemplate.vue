...
export default {
  ...
  render() {
    ...
    const searchForm = $slots.search && $slots.search({params: this.searchParams, getList: this.fetchData})
    ...
    //   scopedSlots: {
    //     default: ({row}) => {
    //       const tableActionSlot = $slots.tableAction && $slots.tableAction({row})
    //       return (
    //           <div>
    //             {this.api.edit ? (
    //                 <el-button size='mini' v-on:click={this.editRow.bind(this, row)}>编辑</el-button>) : null}
    //             {this.api.remove ? (
    //                 <el-button size='mini' v-on:click={this.removeRow.bind(this, row)}>删除</el-button>) : null}
    //             {tableActionSlot}
    //           </div>)
    //     }
    //   }
    // })
    //
    // const showAction = this.api.edit || this.api.remove || $slots.tableAction
    //
    ...
    //     data: this.list
    //   }
    // }, [tableIndex, ...$slots.table(), showAction ? tableAction : null])
    //
    // let dialog
    //
    ...
    //       labelWidth: '80px'
    //     }
    //   }, $slots.form({form: this.form, isEdit: this.form.$$isEdit}))
    //
